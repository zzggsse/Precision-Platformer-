...
**本项目目前未指定开源许可证。** 在你选定并添加 `LICENSE` 文件之前，默认保留所有权利。

建议选择：

| 许可证 | 适合 |
|---|---|
| **MIT** | 最宽松，别人可以随便用，只需保留版权声明。个人项目 / 想被广泛使用推荐这个 |
| Apache-2.0 | 和 MIT 类似，但额外包含专利授权条款。**本项目的依赖 libGDX 用的就是这个** |
| 保留所有权利 | 不开源，仅自己使用或仅发布游戏本体 |

> **注意**：`assets\` 里的 `tiles.png` 和 `kid.png` 是**代码生成的占位美术**
> （见 `tools/GenAssets.java`），可以直接替换。换成第三方素材时注意各自的授权。

选好之后把本节说明换成实际许可证，并在工程根目录加一个 `LICENSE` 文件。
